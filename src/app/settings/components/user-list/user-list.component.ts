...
export class UserListComponent implements OnInit {
    // Table elements
    displayedColumns = ['name', 'cpf', 'email', 'active', 'edit', 'delete'];
    ...
    delete(_id: string) {
        swal({
            text: 'Deseja realmente apagar usuário?',
            type: 'warning',
            ...
            .then((isConfirm) => {
                if (isConfirm) {
                    this.userService.delete(_id)
                        .subscribe((data: any) => {
                            if (data === 'Usuário deletada com sucesso.') {
                                swal({
                                    text: `Usuário deletada com sucesso!`,
                                    type: 'success'
                                }).then(() => {
                                    this.getAll(this.pageIndex, this.pageSize);
                                });
